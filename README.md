...
## 4.ページレイアウト（行数は何で決まるか）
4-1. 「レイアウト -> ページ設定」で、「行数」と「行送り」が設定できる。  
4-2. 「行数 × 行送り 」> 「印刷可能領域（= 用紙サイズ - 上・下余白）」に設定すると、行があふれて、行送りが倍に設定されてしまい、思ったようなレイアウトにならない。  
4-3. A4の例：用紙サイズ「縦297mm × 横210mm」、余白「上35mm、下・右・左30mm」→印刷可能領域「縦232mm × 横150mm」  
4-4. 「ページ設定」で「行数」を試しに「1」にすると、「行送り」は「657.6pt」となる。  
4-5. 「1pt」は「1/72inch」であり、「約0.3528mm」なので、「657.6pt」は「232.00128mm」であり、印刷可能領域と一致する。  
4-6. 「行送り」とは、「1行の背の高さ」である。  
4-7. 「行数」または「行送り」を変更すると、印刷可能領域に収まるように、もう一方が自動調整される。  
4-8. 
