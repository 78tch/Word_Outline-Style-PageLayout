...
Word を推敲の道具として、思考をまとめあげるのに使えるわけです。  
また、Word で文書の見た目を整えるにあたり、「スタイル」機能を使えば、フォント、字の大きさ、太字や下線による強調など、書式設定を一括指定できます。  
少ない作業量で、デザインに統一感のある文書が作成できます。  
さらには、「資料を詰めて１枚に収める」という時に、余白を小さくするという方法以外にも、行数や１行の高さ（行と行の間隔）を指定する、という方法もあります。
